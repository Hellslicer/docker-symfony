...
RUN git clone https://github.com/xdebug/xdebug.git && cd xdebug && git checkout XDEBUG_2_4_0RC2 && phpize && ./configure --enable-xdebug --with-php-config=/usr/bin/php-config7.0 \
    && make && cp modules/xdebug.so /usr/lib/php && echo 'zend_extension=”/usr/lib/php/xdebug.so”' >> /etc/php/7.0/fpm/php.ini

# Configuration
RUN sed -e 's/error_reporting = E_ALL & ~E_DEPRECATED & ~E_STRICT/error_reporting = E_ALL/' -i /etc/php/7.0/cli/php.ini \
    && sed -e 's/error_reporting = E_ALL & ~E_DEPRECATED & ~E_STRICT/error_reporting = E_ALL/' -i /etc/php/7.0/fpm/php.ini \
    ...
    && sed -e 's/pm\.max_children = 5/pm.max_children = 16/' -i /etc/php/7.0/fpm/pool.d/www.conf \
    && sed -e 's/pm\.start_servers = 2/pm.start_servers = 6/' -i /etc/php/7.0/fpm/pool.d/www.conf \
    && sed -e 's/pm\.min_spare_servers = 1/pm.min_spare_servers = 3/' -i /etc/php/7.0/fpm/pool.d/www.conf \
    && sed -e 's/pm\.max_spare_servers = 3/pm.max_spare_servers = 11/' -i /etc/php/7.0/fpm/pool.d/www.conf \
    && sed -e 's/;pm\.max_requests = 500/pm.max_requests = 500/' -i /etc/php/7.0/fpm/pool.d/www.conf \
    && echo "memory_limit=1024M" > /etc/php/7.0/cli/conf.d/memory-limit.ini \
